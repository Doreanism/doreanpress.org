...
// its own.
//
//   /verify/mock?name=Jane%20Doe&handle=janedoe&redirect=/cart
//
...
// rule testable. `handle` is optional, and covers the two shapes a real provider
// comes in: with a public handle (X) and without one (Facebook, LinkedIn).
...
export default defineEventHandler(async (event) => {
  ...
  return completeChallenge(event, {
    ...
    subject,
    name,
    handle: String(query.handle || '').trim().slice(0, 40) || undefined
