...
export function ensureMinistrySchema() {
  if (!schema) schema = (async () => {
    ...
    await sql.transaction([
      ...
      sql`ALTER TABLE book_requests ADD COLUMN IF NOT EXISTS tracker_lease_until timestamptz`,
      sql`CREATE TABLE IF NOT EXISTS ministry_tracking_events (id text PRIMARY KEY, tracker_id text NOT NULL, status text NOT NULL, occurred_at timestamptz NOT NULL)`,
      ...
      sql`CREATE TABLE IF NOT EXISTS ministry_audit (
      ...
      sql`CREATE TABLE IF NOT EXISTS gift_reservations (
      id text PRIMARY KEY, request_id text NOT NULL, items jsonb NOT NULL,
      expires_at timestamptz NOT NULL, created_at timestamptz NOT NULL DEFAULT now())`,
      sql`ALTER TABLE gift_reservations ADD COLUMN IF NOT EXISTS original_items jsonb`,
      sql`ALTER TABLE gift_reservations ADD COLUMN IF NOT EXISTS target_cents integer`,
      sql`CREATE INDEX IF NOT EXISTS gift_reservations_request_idx ON gift_reservations(request_id, expires_at)`,
      sql`CREATE TABLE IF NOT EXISTS ministry_gifts (
      ...
      sql`ALTER TABLE ministry_gifts ADD COLUMN IF NOT EXISTS allocated_cents integer`,
      sql`CREATE TABLE IF NOT EXISTS ministry_outbox (
      ...
      sql`ALTER TABLE ministry_outbox ADD COLUMN IF NOT EXISTS account_id text`
    ])
  })().catch((err) => {
    schema = null
    throw err
...
// Middleware and handlers share a check only within this HTTP request. A new
// request always checks the current role, so revocation takes effect immediately.
const adminChecks = new WeakMap<H3Event, Promise<SignedIn>>()
export function requireAdministrator(event: H3Event) {
  let check = adminChecks.get(event)
  if (!check) {
    check = checkAdministrator(event)
    adminChecks.set(event, check)
  }
  return check
}

async function checkAdministrator(event: H3Event) {
